var PractitionerListView = function(a) {
	...
	this.params.Q_blacklist_S_EQ = '0';
	if(!isGranted("__ALL")){
...
Ext.extend(PractitionerListView, Knight.ux.SearchGridPanel, {
	...
	initTopBarActionItems : function() {
		...
		if (isGranted("_PractitionerBack")) {
			tbarItems.push({
				iconCls : "btn-head-recover",
				text : "复职",
		...
		if (isGranted("_PractitionerMoveList")) {
			tbarItems.push({
				iconCls : "btn-head-cancel",
				text : "加入黑名单",
	...
	moveListPractitioner : function() {
		this.speciallyGridAction(this.dataGridPanel, "practiId", __ctxPath + "/archive/moveListPractitioner.do?target=1", "加入黑名单");
	},
