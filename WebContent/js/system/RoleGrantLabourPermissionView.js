var RoleGrantLabourPermissionView = function(c, a) {
	var b = new Ext.ux.tree.CheckTreePanel({
		title : "为角色[" + a + "]授权",
		id : "RoleGrantLabourPermissionView",
		...
		tools : [ {
			...
			qtip : "重新加载树",
			handler : function() {
				Ext.getCmp("RoleGrantLabourPermissionView").getRootNode().reload();
			}
		} ]
	});
			...
			afterlayout : function() {
				Ext.Ajax.request({
					...
					success : function(e, g) {
						...
						var rights = resp.data[0].labourPermission
			...
			handler : function() {
				...
				Ext.Ajax.request({
					...
					success : function(e, f) {
						...
						Ext.getCmp("RoleGrantLabourPermissionView").close();
					},
					failure : function(e, f) {
						Ext.MessageBox.hide();
						$toast("授权出错，请联系管理员！");
