...
<body>
<c:set var="countNumber" value="${fn:length(returnGoods.returnListSet)}" scope="application"></c:set>
<c:set var="count" value="${((fn:length(returnGoods.returnListSet))-(fn:length(returnGoods.returnListSet)%9))/9}" scope="application"></c:set>
...
<div class="main_detail_0523">
	...
	<c:if test="${(fn:length(returnGoods.returnListSet)!=0)&&(fn:length(returnGoods.returnListSet)%9==0)}">
		<c:set var="countNumber" value="${fn:length(returnGoods.returnListSet)-1}" scope="application"></c:set>
	</c:if>
				...
				<td height="120px">
						...
						<tr>
							<td  width="30%"  rowspan="2" >
								<table width="100%" height="100%">
									...
									<tr>
										<td width="38%"></td>
										<td width="24%" align="center" style="border: solid 1.5px;font-size: 18px;">退货</td>
										<td width="38%"></td>
									</tr>
									<tr></tr>
								</table>
							</td>
							<td  width="30%" align="center"  rowspan="2"><font size=3px style="font-weight:bold;">天元建设集团有限公司<br>周转材料租借退货单</font></td>
							<td rowspan="2">
								<div >
									<img src='<%=request.getContextPath() %>/image-widget?method=qrcode&contents={relateId:${returnGoods.returnId};relateModule : RETURN_GOODS}' height=80 width=89/>
								</div>
							</td>
					...
					<table width="100%" class="tabp" border="0" align="center" cellpadding="0" cellspacing="0"  >
						<tr style="border-bottom: 1px">
							...
							<td class="tdp">&nbsp;${returnGoods.leaseContract.leaseIdentifier}</td>
							<th class="tdp">退货时间:</th>
							<td class="tdp">&nbsp;${returnGoods.returnDate}</td>
							<th class="tdp">退货单号:</th>
							<td class="tdp">&nbsp;${returnGoods.returnSerial}</td>
						</tr>
						<tr style="border-bottom: 1px">	
							<th class="tdp">租借单位:</th>
							<td class="tdp" >&nbsp;${returnGoods.leaseUnit}</td>
							<th class="tdp">项目名称:</th>
							<td class="tdp" colspan="4">&nbsp;${returnGoods.project.projectName}</td>
						</tr>
<%-- 						<tr style="border-bottom: 1px">
							<th width="10%" class="tdp">租借合同:</th>
							<td width="25%" class="tdp">&nbsp;${returnGoods.leaseContract.leaseIdentifier}</td>
							<th width="10%" class="tdp">退货时间:</th>
							<td width="20%" class="tdp">&nbsp;${returnGoods.returnDate}</td>
							<th width="10%" class="tdp">退货单号:</th>
							<td width="25%" class="tdp">&nbsp;${returnGoods.returnSerial}</td>
						</tr>
						<tr style="border-bottom: 1px">	
							<th width="10%" class="tdp">租借单位:</th>
							<td width="15%" class="tdp" >&nbsp;${returnGoods.leaseUnit}</td>
							<th width="10%" class="tdp">项目名称:</th>
							<td width="65%" class="tdp" colspan="4">&nbsp;${returnGoods.project.projectName}</td>
						</tr> --%>
					</table>
				</td>
			</tr>
			<tr valign="top">
				<td height="216px">
					<table width="100%" class="tabp"  border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px">
						...
						<c:forEach var="element" items="${returnGoods.returnListSet}" varStatus="status" begin="${9*v}" end="${9*v+8 }">
							<tr>
			...
			<tr valign="bottom">
				<td height="34px">
					...
					<table  width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px;border-spacing: 0px" >
						<tr>
							<td class="tda" width="15%"><font style="font-weight:bold;">运输人：</font>&nbsp;${returnGoods.transportPersonnel}</td>
							<td class="tda" width="21%"><font style="font-weight:bold;">车牌号：</font>&nbsp;${returnGoods.transportVehicle}</td>
							<td class="tda"><font style="font-weight:bold;">承租单位：</font></td>
							<td class="tda" width="21%"><font style="font-weight:bold;">签名：</font></td>
							<td class="tda" width="15%"><font style="font-weight:bold;">制单人：</font>&nbsp;${returnGoods.userName}</td>
						</tr>
					</table>
				</td>
			</tr>
