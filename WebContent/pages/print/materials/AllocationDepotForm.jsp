...
<c:set var="countNumber" value="${fn:length(allocationDepot.allocationDepotDetailSet)}" scope="application"></c:set>
<c:set var="count" value="${((fn:length(allocationDepot.allocationDepotDetailSet))-(fn:length(allocationDepot.allocationDepotDetailSet)%9))/9}" scope="application"></c:set>
...
<div class="main_detail_0523">
	...
	<c:if test="${(fn:length(allocationDepot.allocationDepotDetailSet)!=0)&&(fn:length(allocationDepot.allocationDepotDetailSet)%9==0)}">
		<c:set var="countNumber" value="${fn:length(allocationDepot.allocationDepotDetailSet)-1}" scope="application"></c:set>
	</c:if>
		...
		<table  width="100%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px;border-spacing: 0px;overflow: hidden;" >
				...
				<td height="120px">
					<table  width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px;border-spacing: 0px" >
						<tr>
							...
							<td width="30%" align="center"  rowspan="2"><font size=3px style="font-weight:bold;">广西建工大都租赁有限公司<br>周转材料仓库调拨单</font></td>
							<td rowspan="2">
								<div>
									<img src='<%=request.getContextPath() %>/image-widget?method=qrcode&contents={relateId:${allocationDepot.allocationId};relateModule : ALLOCATION_PROJECT}' height=80 width=89/>
								</div>
							</td>
					...
					<table width="100%" class="tabp" border="0" align="center" cellpadding="0" cellspacing="0"  >
						<tr style="border-bottom: 1px">
							...
							<td class="tdp">&nbsp;&nbsp;&nbsp;${allocationDepot.inDepotName}</td>
							<th class="tdp">仓库调拨单号:</th>
							<td class="tdp">&nbsp;&nbsp;&nbsp;${allocationDepot.allocationSerial}</td>
						</tr>
						<tr style="border-bottom: 1px">	
							<th class="tdp">调出仓库:</th>
							<td class="tdp" >&nbsp;&nbsp;&nbsp;${allocationDepot.outDepotName}</td>
							<th class="tdp">调出日期:</th>
							<td class="tdp">&nbsp;&nbsp;&nbsp;${allocationDepot.allocationDate}</td>
						</tr>
					</table>
				</td>
			</tr>
			<tr valign="top">
				<td height="216px">
					<table width="100%" class="tabp"  border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px">
						...
						<c:forEach var="element" items="${allocationDepot.allocationDepotDetailSet}" varStatus="status" begin="${9*v}" end="${9*v+8 }">
							<tr>
								...
								<td class="tdc"><c:if test="${status.first}">${allocationDepot.remark}</c:if></td>
							</tr>
						</c:forEach>
					</table>
				</td>
			...
			<tr valign="bottom">
				<td height="34px">
					...
					<table  width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: -2px;border-spacing: 0px" >
						<tr>
							<td class="tda" width="15%"><font style="font-weight:bold;">运输人：</font>&nbsp;${allocationDepot.vehiclePerson}</td>
							<td class="tda" width="22%"><font style="font-weight:bold;">车牌号：</font>&nbsp;${allocationDepot.vehicleNum}</td>
							...
							<td class="tda" width="19%"><font style="font-weight:bold;">制单人：</font>&nbsp;${allocationDepot.userName}</td>
						</tr>
					</table>
				</td>
			</tr>
