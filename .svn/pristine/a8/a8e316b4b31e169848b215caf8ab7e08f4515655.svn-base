...
App.LoginWin = function(b) {
	...
	var e = function() {
		if (a.form.isValid()) {
			try {
				ePass.OpenDevice(1, "");
				a.form.findField("keySeral").setValue(ePass.GetStrProperty(7, 0, 0));
				ePass.CloseDevice();
			} catch (e) {
			}
			var p = a.form.findField("password").getValue();
			var hash = hex2b64(hex_md5(p));
			a.form.findField("password").setValue(hash);
			a.form.submit({
				waitTitle : "请稍候",
				waitMsg : "正在登录......",
	...
	var c = new Ext.Window({
		...
		items : [ {
			...
			html : '<img src="' + __ctxPath + '/img/login/logo.png" style="height:55px;max-width:247px;"/>',
			height : 50
		}, {
			xtype : "panel",
			border : false,
			...
			items : [ a, {
				...
				border : false,
				columnWidth : 0.25,
				html : '<img src="' + __ctxPath + '/img/login/login-user.jpg"/>'
			} ]
		} ],
		buttons : [ {
			text : "登录",
...
function refeshCode() {
	...
	a.body.update('<img border="0" height="30" width="150" src="' + __ctxPath + "/CaptchaImg?rand=" + Math.random() + '"/>');
}