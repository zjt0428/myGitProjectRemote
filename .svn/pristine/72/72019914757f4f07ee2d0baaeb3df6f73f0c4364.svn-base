...
Ext.ux.Exporter.Button = Ext.extend(Ext.Button, {
		...
		if (this.store && Ext.isFunction(this.store.on)) {
			var setLink = function() {
				this.getEl().child('a', true).href = 'data:application/vnd.ms-excel;base64,'
						+ Ext.ux.Exporter[config.exportFunction](this.component, null, config);
  ...
  template: new Ext.Template(
    ...
    '<td class="x-btn-left"><i></i></td><td class="x-btn-center"><i></i></td><td class="x-btn-right"><a class="x-btn-text" href="{1}" target="{2}">{0}</a></td>',
    "</tr></tbody></table>"),
