var SettleContractForm = function(a, b) {
	Ext.apply(this, a || {});
	Ext.apply(this, b || {});
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					value : Env.date.getMonthStartStop(new Date())[0]
				}, {
					readOnly : true,
					hidden: true,
				...
				items : [ {
					...
					value : Env.date.getMonthStartStop(new Date())[1]
				}, {
					width : 200,
					readOnly : true,
					fieldLabel : ContractLeaseFormConfigure.contractSerialHeader,
...
//					value : taxRateData[0]
					...
					editable : false,
					fieldLabel : "结算日期",
					name : "settleContract.settleDate",
					value : this.settleId?this.settleDate:Env.date.getCurrentMonth()[1]
...
//			maxLength : 3000,
			height : 68,
//			maxLengthText : "消息内容只允许输入3000个中文字符",
			fieldLabel : "备注",
	...
	SettleContractForm.superclass.constructor.call(this, {
		title : "结算单明细",
		...
		form_config : {
			...
			saveable : this.saveable,
			url : __ctxPath + "/dispatch/saveSettleContract.do",
			items : items,
...
Ext.extend(SettleContractForm, Knight.ux.FormPanelWindow, {
	...
	saveFormData : function() {
		var startDate = Ext.getCmp("startSettleDate").getValue();
		var endDate = Ext.getCmp("endSettleDate").getValue();
		if (startDate > endDate) {
			Ext.Msg.alert("操作信息", "截止日期小于开始日期!");
			return;
		}
...
//					
	...
	synchronizeDate : function(grid) {
		var endSettleDate = this.getFieldValue("endSettleDate");
		var startSettleDate = this.getFieldValue("startSettleDate");
		if (grid.getSelectionModel().hasSelection()) {
			var dateSynchronizeList = grid.getSelectionModel().getSelections();
			if( !Ext.isEmpty(startSettleDate) && !Ext.isEmpty(endSettleDate) ) {
				for (var i = 0; i < dateSynchronizeList.length; i++) {
					dateSynchronizeList[i].set("startSettleDate", startSettleDate);
					dateSynchronizeList[i].set("endSettleDate", endSettleDate);
				}
			} 
		} else {
	        Ext.MessageBox.show({
