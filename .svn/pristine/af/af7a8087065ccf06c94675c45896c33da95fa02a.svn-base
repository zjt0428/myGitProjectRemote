/**
 *====================================================
 * 文件名称: ReturnGoods.java
 * 修订记录：
 * No    日期				作者(操作:具体内容)
 * 1.    2017年8月25日		陈光毅(创建:创建文件)
 *====================================================
 * 类描述：(说明未实现或其它不应生成javadoc的内容)
 */
package com.knight.emms.dao;

import com.google.inject.ImplementedBy;
import com.knight.emms.core.dao.BaseBusinessModelDao;
import com.knight.emms.dao.impl.ReturnGoodsDaoImpl;
import com.knight.emms.model.ReturnGoods;

/**
 * @ClassName: ReturnGoods
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author 陈光毅
 * @date 2017年8月25日
 */
@ImplementedBy(ReturnGoodsDaoImpl.class)
public interface ReturnGoodsDao extends BaseBusinessModelDao<ReturnGoods> {

}
