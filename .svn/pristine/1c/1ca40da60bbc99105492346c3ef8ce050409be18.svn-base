var LostCompensationDetailGrid = function(a, b) {
	Ext.apply(this, a || {});
	Ext.apply(this, b || {});
	...
	var columns = [ {
		...
		renderer : function(value,p,record){
			...
			if(Number(record.data.lostQuantity)>Number(record.data.oweQuantity)&&record.data.lostQuantity!=0) {
				...
				return record.data.lostQuantity = record.data.oweQuantity;
			}
			return value;
		}
	}, {
	...
	LostCompensationDetailGrid.superclass.constructor.call(this, Ext.apply({
		saveable : this.saveable,
		selectable : this.selectable,
		addForbidden :true,
		fields : LostCompensationDetailListViewField,
		title : "丢失周材明细",
		option : "丢失周材明细",
		tbarItems : tbarItems,
		height : this.height,
		columns : columns,
		delurl : __ctxPath + "/materials/multiDelDetailLostCompensation.do"
	}, this.viewConfig || {}));
};
Ext.extend(LostCompensationDetailGrid, Knight.ux.SubModuleBaseGrid, {
	allLost : function() {
	...
	calculate : function() {
		...
		Ext.getCmp("lostCompensation_totalCompensation").setValue(sum.toFixed(2));
	},
...
//				QUERY_FILTER : "projectMaterialsStoreASC",
//				limit : 500
//			},
...
//		});
		...
		//改为根据丢失赔偿时间获取周材项目库存
		$request({
			url :  __ctxPath + "/materials/getStoreByCompensationDateProjectMaterialsStore.do",
			params : {
				"contractId" : this.contractId,
				"compensationDate" : Ext.getCmp("compensationDate").getValue()
			},
			success : function(g,h) {
				var data = Ext.util.JSON.decode(g.responseText).result;
