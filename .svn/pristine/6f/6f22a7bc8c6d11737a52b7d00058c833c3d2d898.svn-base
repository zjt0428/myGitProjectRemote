/**
 * 版权所有：日升建机信息科技有限公司
 * Copyright 2013 Risit Construction Machinery Information Technology Co., Ltd.
 *====================================================
 * 文件名称: SettleItemBriefDaoImpl.java
 * 修订记录：
 * No    日期				作者(操作:具体内容)
 * 1.    2014-1-23			chenxy(创建:创建文件)
 *====================================================
 * 类描述：(说明未实现或其它不应生成javadoc的内容)
 */
package com.knight.emms.dao.impl;

import com.knight.core.dao.impl.BaseLongPKDaoImpl;
import com.knight.emms.dao.SettleItemBriefDao;
import com.knight.emms.model.SettleItemBrief;

/**
 * @ClassName: SettleItemBriefDaoImpl
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author chenxy
 * @date 2014-1-23 下午4:25:09
 */
public class SettleItemBriefDaoImpl extends BaseLongPKDaoImpl<SettleItemBrief> implements SettleItemBriefDao {

}
