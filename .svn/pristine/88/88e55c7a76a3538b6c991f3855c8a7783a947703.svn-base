	...
	<script type="text/javascript">
		...
		function doSave(){
			if (confirm("是否提交合同信息？")) {
				...
				document.form1.action="editContentContractLease.do?formpage=ContractLease&EVENT=SAVE&contractId=<%=contractId%>";
		...
		function doPrint(){
			var content = UE.getEditor('editor').getContent();
			document.getElementById("content1").value=content;
			document.form1.action="editContentContractLease.do?formpage=ContractLease&EVENT=PRINT&contractId=<%=contractId%>";
			document.form1.submit();
		}
