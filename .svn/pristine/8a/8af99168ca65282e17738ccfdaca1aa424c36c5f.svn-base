var LabourDepartmentView = function(a) {
	Ext.apply(this, a);
	this.params = {};
	...
	this.departmentTree = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			url : __ctxPath + "/system/listLabourDepartment.do"
		}),
	...
	var menuItems = [];
	if (isGranted("_LabDepartmentAdd")) {
		menuItems.push({
			text : "新建部门",
			iconCls : "menu-add",
	...
	if (isGranted("_LabDepartmentEdit")) {
		menuItems.push({
			text : "修改部门信息",
			iconCls : "menu-edit",
	...
	if (isGranted("_LabDepartmentDel")) {
		menuItems.push({
			text : "删除部门",
			iconCls : "menu-del",
	...
	LabourDepartmentView.superclass.constructor.call(this, Ext.apply({
		id : "LabourDepartmentView",
		title : "部门信息",
...
Ext.extend(LabourDepartmentView, Knight.ux.SearchGridPanel, {
