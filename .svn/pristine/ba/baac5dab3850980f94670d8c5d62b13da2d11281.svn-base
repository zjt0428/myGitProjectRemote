var EquipmentDistributionView = function(a, b) {
	Ext.apply(this, a || {});
	Ext.apply(this, b || {});
	this.params = {};
	...
	var actionItems = [ {
		...
		hidden : true,
		qtip : "辖区",
		handler : this.drillingDistributeEquipment.createDelegate(this)
	}, {
		iconCls : "btn-grid-read",
		hidden : true,
		qtip : "明细",
		handler : this.showDistributeEquipment.createDelegate(this)
	} ];
	var tbarItems = [ {
		iconCls : "btn-mail_back",
		text : "返回",
		handler : this.higherLevelDistributeEquipment.createDelegate(this)
	} ];
	var datagrid_config = {
		checkboxHidden : true,
		store : {
	...
	this.distributionGridPanel = new Knight.ux.SearchGridPanel(Ext.apply({
		...
		collapsed : false,
		collapseMode : "mini",
		url : __ctxPath + "/archive/distributeEquipment.do",
	...
	EquipmentDistributionView.superclass.constructor.call(this, {
		id : "EquipmentDistributionView",
		layout : "border",
		title : "设备分布总览",
...
Ext.extend(EquipmentDistributionView, Ext.Panel, {
	...
	// 下级地区
	drillingDistributeEquipment : function(record) {
		var zoom = 5;
	...
	// 上一级
	higherLevelDistributeEquipment : function() {
		this.distributeAreaType = this.distributeAreaType - 1;
		var zoom = 5;
		if (this.distributeAreaType == "0") {
			this.province = {};
	...
	showDistributeEquipment : function(record) {
		var params = {};
...
//		params["Q_[project.delFlag]_S_EQ"] = 1
