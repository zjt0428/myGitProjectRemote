var InsideSettlementListGrid = function (a, b) {
	Ext.apply(this, a || {});
	Ext.apply(this, b || {});
	...
	InsideSettlementListGrid.superclass.constructor.call(this, Ext.apply({
		...
		title : "内部结算清单",
		option : "租借结算",
		fields : InsideSettlementListListViewField,
		tbarItems :this.tbarItems,
		columns : columns,
		delurl : __ctxPath + "/materials/multiDelListLeaseSettlement.do"
	}, this.grid_config || {}));
}
Ext.extend(InsideSettlementListGrid, Knight.ux.SubModuleBaseGrid, {
	...
	oppositeNumber : function(data) {
		...
		if (arr.indexOf(data.receiptType)>=0) {
			total = ((Number(data.quantity) * (Number(data.convertedQuantity) * 10e5) * (Number(data.dailyRent) * 10e5) * Number(data.conversionDays)) / 10e5) / 10e5;
		} else {
			total = ((Number(data.quantity) * (Number(data.convertedQuantity) * 10e5) * (Number(0 - Math.abs(data.dailyRent)) * 10e5) * Number(data.conversionDays)) / 10e5) / 10e5;
		}
		//内部结算，负数不抵扣租金
//		if(data.quantity<0) {
//			total=0;
