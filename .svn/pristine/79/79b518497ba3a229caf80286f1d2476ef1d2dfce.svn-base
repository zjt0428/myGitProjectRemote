var ContractMaterialsForm2 = function(a, b) {
	Ext.apply(this, a || {});
	Ext.apply(this, b || {});
	...
	var belongToAreaCombo = $initComboBoxField("工程区域", "contractMaterials.belongToArea", "belongToArea", {
		editable : true,
	...
	var assetsPropertyCombo = $initComboBoxField("资产属性", "contractMaterials.assetsProperty", "assetsProperty", {
		defaultValueIndex : 1,
		editable : true
	});
	var projectTypeCombo = $initComboBoxField("工程类别", "contractMaterials.projectType", "projectType", {
		defaultValueIndex : 0,
		editable : true
	});
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					fields : [ "projectId", "projectName","address","taxCaculateType","taxCaculateTypeName" ],
...
//				},
				...
				items : [ projectTypeCombo,
					...
					fields : [ "pbEnt", "pbModule", "pbEntName", "pbEntLinkMan" ],
...
//		hidden : !this.saveable,
	...
	items.push(remarks);
	ContractMaterialsForm2.superclass.constructor.call(this, {
		title : "租赁合同明细",
		maximized : true,
...
Ext.extend(ContractMaterialsForm2, Knight.ux.FormPanelWindow, {
	...
	importProjectArchives : function(data, fields) {
		this.setMultiFieldValue(fields, [ data.projectId, data.projectName, data.address,data.taxMode,data.taxModeName ]);
	},
	...
	loadFormData : function() {
		...
		if(Ext.isEmpty(this.data1)){
			...
			if(!Ext.isEmpty(this.data1.materialsPlanId)){
				var data = this.data1;
				this.setMultiFieldValue([ "assetsPropertyName", "projectTypeName" ], [ data.assetsPropertyName, data.projectTypeName ]);
				this.setMultiFieldValue([ "projectId", "projectName" ], [ data.projectId, data.projectName ]);
				this.setFieldValue([ "belongToAreaName"],[ data.belongToAreaName])
			}
	    }
	}	
});