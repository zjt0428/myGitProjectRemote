...
Ext.extend(Ext.ux.Utils.FSA, Ext.ux.Utils.EventQueue, {
	...
	processEvent : function(a, d) {
		...
		if (!h) {
			throw "State '" + this.current_state + "' has no transition for event '" + a + "'.";
		}
		...
		throw "State '" + this.current_state + "' has no transition for event '" + a + "' in current context";
	},
...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button, {
	...
	createInputFile : function() {
		var c = this.el;
		c.position("relative");
		this.input_file = Ext.DomHelper.insertAfter(c, {
			tag : "input",
			type : "file",
			size : 1,
		...
		var b = c.getBox();
		this.input_file.setStyle("font-size", (b.width * 0.5) + "px");
		...
		this.input_file.setTop(b.height - d.height + a.y + "px");
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	fillToolbar : function() {
		...
		a.add("-");
		a.x_buttons.indicator = a.addItem(new Ext.Toolbar.Item(Ext.DomHelper.append(a.getEl(), {
	...
	renderFilenameCell : function(i, a, c, e, h, d) {
		...
		var g = function() {
			try {
				Ext.fly(b.getCell(e, h)).child(".x-grid3-cell-inner").dom["qtip"] = i;
			} catch (f) {
			}
		};
		g.defer(1000);
	...
	isPermittedFile : function(c) {
		...
		if (this.isPermittedFileType(b)) {
			...
			Ext.Msg.alert(this.i18n.error_msgbox_title, String.format(this.i18n.err_file_type_not_permitted, b, this.permitted_extensions.join(this.i18n.permitted_extensions_join_str)));
			a = false;
		}
		return a;
	},
	...
	updateProgressBar : function() {
		if (this.is_uploading) {
			...
			this.progress_bar.updateProgress(a, String.format(this.i18n.progress_uploading_text, this.initial_queued_count - b, this.initial_queued_count));
		} else {
	...
	updateToolbar : function() {
		...
		if (this.is_uploading) {
			...
			Ext.fly(a.x_buttons.indicator.getEl()).replaceClass("ext-ux-uploaddialog-indicator-stoped", "ext-ux-uploaddialog-indicator-processing");
			a.x_buttons.upload.setIconClass("ext-ux-uploaddialog-uploadstopbtn");
			...
			Ext.fly(a.x_buttons.indicator.getEl()).replaceClass("ext-ux-uploaddialog-indicator-processing", "ext-ux-uploaddialog-indicator-stoped");
			a.x_buttons.upload.setIconClass("ext-ux-uploaddialog-uploadstartbtn");
	...
	prepareNextUploadTask : function() {
		...
		var a = null;
		b.each(function(c) {
			if (!a && c.get("state") == Ext.ux.UploadDialog.FileRecord.STATE_QUEUE) {
				a = c;
			} else {
		...
		a.set("state", Ext.ux.UploadDialog.FileRecord.STATE_PROCESSING);
	...
	updateRecordState : function(a) {
		if ("success" in a.response && a.response.success) {
			a.record.set("state", Ext.ux.UploadDialog.FileRecord.STATE_FINISHED);
			a.record.set("note", a.response.message || a.response.error || this.i18n.note_upload_success);
		} else {
			a.record.set("state", Ext.ux.UploadDialog.FileRecord.STATE_FAILED);
			a.record.set("note", a.response.message || a.response.error || this.i18n.note_upload_error);
		}
		a.record.commit();
	},
	...
	abortUpload : function() {
		...
		a.set("state", Ext.ux.UploadDialog.FileRecord.STATE_FAILED);
	...
	getQueuedCount : function(b) {
		...
		a.each(function(d) {
			...
			if (b && d.get("state") == Ext.ux.UploadDialog.FileRecord.STATE_PROCESSING) {
				c++;
			}
		});
		return c;
