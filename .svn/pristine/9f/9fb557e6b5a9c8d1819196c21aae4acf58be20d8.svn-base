...
Ext.ux.Printer.GridPanelRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
  ...
  generateBody: function(grid) {
    ...
    return String.format('<table style="font-size:12px" border="0" cellspacing="1" cellpadding="0" bgcolor="#000000">{0}<tpl for=".">{1}</tpl></table>', headings, body);
  },
  ...
  getColumns: function(grid) {
    ...
    Ext.each(grid.getColumnModel().config, function(col) {
		if (col.hidden != true && col.dataIndex != "") {
			columns.push(col);
		}
	}, this);
  	return columns;
  ...
  headerTpl:  new Ext.XTemplate(
    '<tr bgcolor="#FFFFFF">',
      '<tpl for=".">',
        '<th align="center" width={width} bgcolor="#FFFFFF">{header}</th>',
      '</tpl>',
    '</tr>'
  ),
  ...
  bodyTpl:  new Ext.XTemplate(
    '<tr bgcolor="#FFFFFF">',
      '<tpl for=".">',
        '<td align="center" bgcolor="#FFFFFF">\{{dataIndex}\}</td>',
      '</tpl>',
    '</tr>'
  )
});
