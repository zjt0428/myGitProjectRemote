...
import com.knight.app.dao.TFlowInstanceDao;
 ...
 */
public class TFlowInstanceServiceImpl  extends BusinessLongPKServiceImpl<TFlowInstance> implements TFlowInstanceService {

	private TFlowInstanceDao tflowInstanceDao;

	public TFlowInstanceServiceImpl(TFlowInstanceDao dao) {
		super(dao);
		this.tflowInstanceDao = dao;
	}

	public void saveOrUpdate(TFlowInstance tflowInstance) {
		if (tflowInstance.getInsid()  == null) {
			tflowInstanceDao.save(tflowInstance);
		}else{
			tflowInstanceDao.merge(tflowInstance);
		}
	}
