var OperatorSalaryStatementGrid = function(a, b) {
	...
	var columns = [{
		...
		renderer : function(v, p, record){
			...
			if(v == "true" || v == true){
				record.data.summary = record.data.salary*record.data.quantity-record.data.deductRent;
				v = '-on';
			}else{
				record.data.summary = "";
				v = '';
		...
		renderer : function(value, metadata, record) {
			if(this.saveable && Ext.isNumber(record.data.summary) && Ext.isNumber(record.data.foodAllowance)&& Ext.isNumber(record.data.deductRent)) {
				record.data.costTotal = (record.data.summary + record.data.foodAllowance).toFixed(2);
...
//		}
		data.rentStandard = 0;
		if(this.parentForm.contract!=null && this.parentForm.contract.contractOperatorFreeSet!=null) {
			var operatorSet = this.parentForm.contract.contractOperatorFreeSet;
			for(var j=0; j<operatorSet.length; j++) {
				if(operatorSet[j].equipSpecificName == data.equipSpecificName) {
					data.rentStandard = operatorSet[j].rent==null?data.rentStandard:operatorSet[j].rent;//租金标准
				}
			}
		}
...
//				var newDate = null;
				var ate = $ajaxSyncCall(__ctxPath + "/equip/getLastSettleDateEquipmentDiary.do",{
					...
					tabFlag : "OperatorSalaryStatement"
				});
				var map = {};
