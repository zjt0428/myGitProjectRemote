JobView = Ext.extend(Ext.Panel, {
	...
	initComponents : function() {
		...
		if (isGranted("_JobEdit")) {
			b.push({
				...
				handler : this.editRecord.createDelegate(this)
			});
		}
		if (isGranted("_JobDel")) {
			b.push({
				...
				qtip : "删除",
				style : "margin:0 3px 0 3px",
				handler : this.delJob.createDelegate(this)
			});
		}
		...
		if (isGranted("_JobRec")) {
			this.topbar.add(new Ext.Button({
				iconCls : "btn-hrm-empProfile-recovery",
				text : "恢复职位",
	...
	delJob : function(grid, rowIndex) {
		var a = grid.getStore().getAt(rowIndex).data.jobId;
		this.delByIds(a);
	},
	...
	editRecord : function(grid, rowIndex) {
		var a = grid.getStore().getAt(rowIndex).data.jobId;
		new JobForm({
			jobId : a
		}).show();
