package com.knight.emms.dao.impl;

import com.knight.emms.core.dao.BaseBusinessModelDaoImpl;
import com.knight.emms.dao.MaterialsDispatchDao;
import com.knight.emms.model.MaterialsDispatch;

/**
* @author 作者 :jlh
* @version 创建时间：2017年7月12日 上午9:35:58
* 类说明
*/
public class MaterialsDispatchDaoImpl extends BaseBusinessModelDaoImpl<MaterialsDispatch> implements MaterialsDispatchDao {

}
