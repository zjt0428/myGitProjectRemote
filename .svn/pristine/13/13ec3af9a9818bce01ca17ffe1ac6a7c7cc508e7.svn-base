		...
		var j = new Ext.grid.ColumnModel({
			columns : [ c, new Ext.grid.RowNumberer(), {
				...
				renderer : function(o, p, n) {
					var q = n.data.sex;
					if (q == 1) {
						return '<img src="' + __ctxPath + '/img/flag/man.png"/>&nbsp;' + o;
					} else {
						return '<img src="' + __ctxPath + '/img/flag/women.png"/>&nbsp;' + o;
					}
				},
				width : 60
			} ],
				...
				handler : function() {
					b.getForm().submit({
						...
						success : function(o, p) {
							k.getStore().proxy.conn.url = __ctxPath + "/system/listAppUser.do";
							var n = Ext.util.JSON.decode(p.response.responseText);
